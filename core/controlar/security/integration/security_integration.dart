...
class SecurityIntegration {
  ...
  /// تمكين تحليلات الأمان
  Future<bool> enableSecurityAnalytics({String? customEndpoint}) async {
    _logger.info("SecurityIntegration", "Enabling security analytics");

    try {
      // تنفيذ تمكين تحليلات الأمان
      if (!_isSecurityInitialized) {
      ...
      if (customEndpoint != null) {
        _logger.info(
            "SecurityIntegration", "Using custom endpoint: $customEndpoint");
        // تكوين نقطة النهاية المخصصة
      }

      return true;
    } catch (e, s) {
      _logger.error(
          "SecurityIntegration", "Failed to enable security analytics", e, s);
      return false;
    }
  }

  /// تحرير موارد تحليلات الأمان
  Future<void> releaseAnalyticsResources() async {
    _logger.info("SecurityIntegration", "Releasing analytics resources");

    try {
      // تحرير الموارد
    } catch (e, s) {
      _logger.error(
          "SecurityIntegration", "Error releasing analytics resources", e, s);
    }
  }
...
final securityIntegrationProvider = Provider<SecurityIntegration>((ref) {
  ...
  final localHardwareManager = ref.watch(localHardwareManagerProvider);
  final regionalComplianceService =
      ref.watch(regionalComplianceServiceProvider);
  final operationTemplateManager = ref.watch(operationTemplateManagerProvider);
  final dataProcessingManager = ref.watch(dataProcessingManagerProvider);
  final localStorageUtility = ref.watch(localStorageUtilityProvider);
  final userVerificationService = ref.watch(userVerificationServiceProvider);
  final deviceId =
