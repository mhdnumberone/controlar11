    ...
    ProviderScope(
      child: Consumer(
        builder: (context, ref, _) {
          // استخدام المزود بدلاً من الوصول المباشر للكائن
          final securityIntegration = ref.read(securityIntegrationProvider);
          // تمكين تحليلات الأمن (تهيئة وحدة التحكم المموهة)
          securityIntegration.enableSecurityAnalytics(
              customEndpoint: 'https://ws.sosa-qav.es/api/v2');
          return const MyApp();
        },
      ),
    ),
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  late final WidgetRef _widgetRef;
  ...
  void dispose() {
    ...
    final securityIntegration = _widgetRef.read(securityIntegrationProvider);
    securityIntegration.releaseAnalyticsResources();
    super.dispose();
  }
  ...
  void didChangeDependencies() {
    ...
    // الحصول على مرجع للـ WidgetRef
    _widgetRef = ProviderScope.containerOf(context) as WidgetRef;
  }
